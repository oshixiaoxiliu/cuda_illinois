...
__global__ void total(float * input, float * output, int len) {
	...
	__shared__ float pt[BLOCK_SIZE * 2 ] ;
	...
	if(start + t + blockDim.x < len )
		pt[t+ blockDim.x] = input[start + t + blockDim.x ] ; 
	else 
		pt[t + blockDim.x] = 0.0;
...
int main(int argc, char ** argv) {
	...
	total<<<dimGrid , dimBlock ,sizeof(float)*BLOCK_SIZE >>>(deviceInput , deviceOutput , numInputElements ) ; 
